...
public class Refactor {
	...
	public List<Opt> simplifyConditionals(List<Opt> conditionals){
			...
			for (int j = i+1; j < conditionals.size(); j++){
				...
				if (iNode.getConditional().equivalentTo(jNode.getConditional())){
					for (int z = 0; z < jNode.getChildren().size(); z++){
						...
						nodesToAdd.add(zNode);
					}
					conditionals.remove(j);
				}
			}
			iNode.setChildren(new ArrayList<Node>());
			for (int z = 0; z < nodesToAdd.size(); z++){
				iNode.addChild(nodesToAdd.get(z));
			}
		}
		return conditionals;
	}
	...
	public void refactorNode (Node node){
		...
		conditionals = this.simplifyConditionals(conditionals);
	...
	public Node createArrayExprStatement(String identifier, String index, Constant value){
